...
function createWrapper(parent,value,x,y,wrapperRank)
    if _G[parent..value.."Wrapper"] == nil then
        ...
        _G[parent..value.."Wrapper"]:SetAlpha(BadRobot_data.BadRobotUI.alpha)
        -- texture part
        ...
        if BadRobot_data.options[GetSpecialization()][value.."Wrapper"] == true then
            _G[parent..value.."Wrapper"].texture:SetTexture(100/255,100/255,100/255,BadRobot_data.BadRobotUI.optionsFrame.color.a)
        else
            _G[parent..value.."Wrapper"].texture:SetTexture(60/255,60/255,60/255,BadRobot_data.BadRobotUI.optionsFrame.color.a)
        end
        ...
        _G[parent..value.."Wrapper"]:SetScript("OnClick",function(self)
            ...
            if BadRobot_data.options[GetSpecialization()][value.."Wrapper"] ~= true then
                BadRobot_data.options[GetSpecialization()][value.."Wrapper"] = true
                replaceWraps(parent,value)
            else
                BadRobot_data.options[GetSpecialization()][value.."Wrapper"] = false
                replaceWraps(parent,value)
            end
        ...
        _G[parent..value.."Wrapper"]:SetScript("OnEnter",function(self)
            _G[parent..value.."Wrapper"].texture:SetTexture(200/255,200/255,200/255,BadRobot_data.BadRobotUI.optionsFrame.color.a)
        end)
        -- leave event
        ...
        _G[parent..value.."Text"]:SetFont(BadRobot_data.BadRobotUI.font,BadRobot_data.BadRobotUI.fontsize,"THICKOUTLINE")
...
function hideRow(parent,i)
    local value = BadRobot_data.options[GetSpecialization()].profile[i].name
...
function replaceWraps(parent)
    ...
    for i = 1, #currentProfile do
        ...
        if BadRobot_data.options[GetSpecialization()][name.."Wrapper"] == true then
            isShown = true
        elseif BadRobot_data.options[GetSpecialization()][name.."Wrapper"] == false then
            isShown = false
        end
    end
    local scale = BadRobot_data.BadRobotUI.optionsFrame.scale or 1
