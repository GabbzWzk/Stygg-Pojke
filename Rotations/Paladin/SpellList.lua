if select(3,UnitClass("player")) == 2 then
  ...
  function castStrike()
    local strike = strike
    if BadRobot_data["AoE"] == 2 or (BadRobot_data["AoE"] == 3 and numberOfTargetsForHammerOfRighteous > 2) or keyPressAoE then  --If Toggle to 2(AoE) or 3(Auto and more then 2 targets, its actually 4 but its just simplier to do aoe
  ...
  --       local SacredShieldCheck = BadRobot_data["Check Sacred Shield"];
  --  -      local SacredShield = BadRobot_data["Box Sacred Shield"];
  ...
  --            if ((isMoving("player") or UnitAffectingCombat("player")) and not UnitBuffID("player",20925)) or (LastVengeance ~= nil and (GetVengeance() > (BadRobot_data["Box Sacred Vengeance"] + LastVengeance))) then
